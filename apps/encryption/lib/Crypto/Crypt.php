...
class Crypt {
	...
	public function __construct(ILogger $logger, IUserSession $userSession, IConfig $config, IL10N $l) {
		...
		$this->useLegacyEncoding = $this->config->getSystemValueBool('encryption.use_legacy_encoding', false);
	}
	...
	public function decryptPrivateKey($privateKey, $password = '', $uid = '') {
		...
		$binaryEncoding = isset($header['encoding']) && $header['encoding'] === self::DEFAULT_ENCODING_FORMAT;
	 ...
	 */
	public function symmetricDecryptFileContent($keyFileContents, $passPhrase, $cipher = self::DEFAULT_CIPHER, $version = 0, $position = 0, $binaryEncoding = false) {
		if ($keyFileContents == '') {
			return '';
		}
	 ...
	 * @throws DecryptionFailedException
	 */
	private function decrypt($encryptedContent, $iv, $passPhrase = '', $cipher = self::DEFAULT_CIPHER, $binaryEncoding = false) {
